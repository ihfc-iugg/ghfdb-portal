...
def test_get_dataset_response_schema(client, admin_approval_dataset):
    ...
    for field in required_fields:
        assert field in data, f"Required field '{field}' missing from response. Breaking change detected!"
    ...
    if data["description"] is not None:
        assert isinstance(data["description"], str), "description must be string or null"

    # Assert: Optional fields (added in later versions)
    # If present, validate type to ensure consistency
    if "updated_date" in data:
        assert isinstance(data["updated_date"], str), "updated_date (if present) must be string (ISO 8601)"
...
def test_get_dataset_list_response_schema(client):
    ...
    for field in pagination_fields:
        assert field in data, f"Pagination field '{field}' missing from list response. Breaking change detected!"
    ...
    assert data["next"] is None or isinstance(data["next"], str), "next must be string (URL) or null"
    assert data["previous"] is None or isinstance(data["previous"], str), "previous must be string (URL) or null"

    assert isinstance(data["results"], list), "results must be array. Changing to object breaks client contracts."
    ...
    if data["results"]:
        ...
        for field in item_required_fields:
            assert field in first_item, f"List item missing required field '{field}'. Breaking change detected!"
...
def test_unauthorized_request_error_contract(client):
    ...
    if response.status_code == 401:
        ...
        assert "detail" in error, "Error response must include 'detail' field. Breaking change detected!"
        ...
        if "error_code" in error:
            ...
            assert error["error_code"] in ["unauthorized", "authentication_failed", "authentication_required"], (
...
def test_nullable_field_explicit_null(client):
    ...
    assert data["description"] is None, "description should be explicit null (not undefined)"

    # Arrange: Create dataset with non-null description
    dataset_with_desc = Dataset.objects.create(
        name="Non-Null Field Test", visibility=1, description="This is a test dataset"
    )
...
def test_pagination_contract(client):
    ...
    for i in range(15):
        Dataset.objects.create(name=f"Pagination Test Dataset {i}", visibility=1)
    ...
    assert isinstance(data["next"], str), "next must be string (URL) when more pages exist"
    assert "page" in data["next"] or "offset" in data["next"], "next URL should include pagination parameter"

    # Assert: previous is null (on first page)
    assert data["previous"] is None, "previous must be null on first page"

    # Assert: results length matches page_size
    assert len(data["results"]) == 10, f"Expected 10 results per page, got {len(data['results'])}"
    ...
    if data["next"]:
        ...
        assert isinstance(data2["previous"], str), "previous must be string (URL) when not on first page"

        # Assert: count remains consistent across pages
        assert data2["count"] == data["count"], "count must be consistent across all pages"
