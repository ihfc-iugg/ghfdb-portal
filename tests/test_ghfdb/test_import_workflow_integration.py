...
def test_import_minimal_dataset_happy_path(minimal_ghfdb_import_data):
    ...
    assert not result.has_validation_errors(), f"Validation errors: {result.validation_errors}"
...
def test_import_invalid_dataset_raises_validation_errors(invalid_ghfdb_import_data):
    ...
    """
    # Arrange: Create empty dataset
    dataset = Dataset.objects.create(name="Invalid Dataset Integration Test", visibility=0)
    ...
    assert result.has_errors() or result.has_validation_errors(), "Import should detect errors from invalid fixture"

    # Assert: No records created on validation failure
    sites = HeatFlowSite.objects.filter(dataset=dataset)
    assert sites.count() == 0, "No database records should be created when validation fails"
...
def test_export_without_approval_fails():
    ...
    HeatFlowSite.objects.create(dataset=dataset, name="Test Site 1", lat=45.0, lon=-120.0)
    HeatFlowSite.objects.create(dataset=dataset, name="Test Site 2", lat=46.0, lon=-121.0)
    ...
    assert exported_rows == 0, "Private datasets should not be exportable until approved"
...
def test_full_workflow_import_to_export(minimal_ghfdb_import_data):
    ...
    admin_user = User.objects.create_user(
        username="admin_integration", email="admin@example.com", is_staff=True, is_superuser=True
    )
    ...
    exported_rows = len(export_data.dict) if hasattr(export_data, "dict") else 0
    assert exported_rows == 5, f"Export should contain all 5 imported sites, got {exported_rows}"
