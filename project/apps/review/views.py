...
from geoluminate.contrib.literature.views import PublicationList
from geoluminate.contrib.user.forms import SocialSignupForm

from .models import HeatFlow, HeatFlowReview, Interval, IntervalReview, Review
...
class BecomeReviewer(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not user.orcid:
            ...
            forms.append(
                (_("Account Connections"), "socialaccount/connections.html", None)
            )
...
def nominate_to_review_publication(request, pk):
    ...
    form = ReviewForm(
        {"reviewer": request.user, "publication": get_object_or_404(Publication, pk=pk)}
    )
    if form.is_valid():
        review = form.save()
