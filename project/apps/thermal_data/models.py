...
from well_logs.models import Data, Log
...
class Conductivity(Data):
    ...
    sample_type = models.CharField(
        _("sample_type"),
        help_text=_("Type of sample used to determine conductivity"),
        max_length=23,
        choices=choices.ConductivitySource.choices,
    )
    method = models.CharField(
        _("thermal conductivity method"),
        help_text=_(
    ...
    saturation_state = models.CharField(
        _("thermal conductivity saturation"),
        help_text=_("Saturation state of the sample"),
        max_length=100,
    )

    lithology = models.OneToOneField(
        "geoscience.EarthMaterial",
    ...
    name = models.CharField(_("sample name/ID"), max_length=200, blank=True)
    length = models.FloatField(_("length (cm)"), null=True, blank=True)
    width = models.FloatField(_("width (cm)"), null=True, blank=True)
    diameter = models.FloatField(_("diameter (cm)"), null=True, blank=True)
    thickness = models.FloatField(_("thickness (cm)"), null=True, blank=True)

    orientation = RangeField(
        _("orientation"),
        help_text=_(
...
class TemperatureLog(AbstractLog, Log):
    ...
    correction = models.CharField(
        _("correction type"), max_length=150, blank=True, null=True
    )
