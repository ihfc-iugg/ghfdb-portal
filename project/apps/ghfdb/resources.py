...
from .widgets import CorrectionField, CorrectionsWidget, ReferenceWidget, SiteWidget
...
class SiteResource(ResourceMixin):
    class Meta:
        model = HeatFlow
        exclude = ["slug", "date_added", "id"] + [
            f.name for f in model._meta.related_objects
        ]
        import_id_fields = ["name", "lat", "lon"]
        widgets = {
            "q_date_acq": {"format": "%Y-%m"},
        }
        # skip_diff=True
        # use_bulk = True
...
class IntervalResource(ResourceMixin):
    ...
    site = Field(attribute="site", widget=SiteWidget(HeatFlow))
    ...
    def clean_date(self, row):
        if row.get("q_date_acq"):
            date_parts = str(row["q_date_acq"]).split("-")

            # for format YYYY-MM
            if len(date_parts) == 2:
                row["q_date_acq"] = f'{row["q_date_acq"]}-01'

            # for format YYYY only
            elif len(date_parts) == 1:
                row["q_date_acq"] = f'{row["q_date_acq"]}-01-01'
