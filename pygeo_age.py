...
def query_cmap(cmap, num, range_min, range_max):
    cmap = plt.cm.get_cmap(cmap)
    rangex = range_max-range_min
    val=num-range_min
    
    rgb = cmap(val/rangex)
    rgb = [int(255*x) for x in rgb[0:3]]
...
def box_plot():
    x = list(GEO_AGE['eon'].values())
    labels = list(GEO_AGE['eon'].keys())

    intervals = pd.IntervalIndex.from_tuples(x)

    df = pd.read_csv('heat_flow.csv')
    age_groups = df.groupby(pd.cut(
        df['site__province__juvenile_age_max'],intervals,precision=0))

    fig = go.Figure()

    for i, group in enumerate(age_groups):
        key, age = group[0],group[1]

        age_stats = age.heat_flow.agg(
                dict(
                        q3=lambda x: x.quantile(.75),
                        q1=lambda x: x.quantile(.25),
                        mean=lambda x: x.mean(),
                        median=lambda x: x.median(),
                        upperfence=lambda x: x.quantile(.95),
                        lowerfence=lambda x: x.quantile(.05),
                    )               
                )
        ...
        if age_stats.empty:
            ...
            c = query_cmap('coolwarm',age_stats.loc['mean'],40,80)
            x = {k: [v] for k,v in age_stats.to_dict().items()}
    ...
    fig.write_html('plotly_test.html',include_plotlyjs='cdn',full_html=True) 


def suburst():
    df = pd.read_csv('heat_flow.csv')

    age = 'site__province__tectonic_age_max'

    for age_type in ['eon','era','period','epoch']:
