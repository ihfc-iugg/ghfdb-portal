...
- [X] T079 [P] [US2] Write failing regression tests in `tests/test_ghfdb/test_resources/test_widgets.py` for numeric cell input (BUG-007): (1) `ConceptWidget.clean(42, row={})` raises `ValueError` whose message contains the vocabulary class name and the value `42` — **not** `AttributeError`; (2) `RelatedModelWidget` subclass with `sentinel_column` set, called with `row={sentinel_column: 1}`, raises `ValueError` that names the sentinel column; (3) `ParentWidget.clean("", row={"name": 1, ...})` raises `ValueError` naming `"name"` column; all `ValueError` messages must be strings (not `AttributeError` propagation)
- [X] T080 [US2] Add `try/except AttributeError` type-guards in `project/ghfdb/resources/_widgets.py` for all three affected call sites (BUG-007): (1) in `ConceptWidget.clean()` wrap `super().clean(value, row, **kwargs)` — catch `AttributeError`, re-raise `ValueError(_('Column value %(val)r is not a text string; expected a vocabulary label for %(vocab)s.') % {"val": value, "vocab": self.vocabulary.__name__})`; (2) in `RelatedModelWidget.clean()` wrap the sentinel `.strip()` call — catch `AttributeError`, re-raise `ValueError(_('Column \'%(col)s\' contains a non-text value %(val)r; expected a text string.') % {"col": self.sentinel_column, "val": raw_sentinel})`; (3) in `ParentWidget.clean()` wrap the `"name"` sentinel `.strip()` call — catch `AttributeError`, re-raise `ValueError(_('Column \'name\' contains a non-text value %(val)r; expected a site name string.') % {"val": raw_name})`; all messages MUST use `gettext_lazy()` (constitution §V)
