...
- **Template-aware upsert**: `import_id_fields` uses always-present natural keys; `ID`/`ID_parent` are stored as `local_id` when present but never used as the sole upsert discriminator for standard uploads.
