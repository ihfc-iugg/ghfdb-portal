...
from geoluminate.widgets import ChoiceManyToMany
...
class SiteWidget(ForeignKeyWidget):
    ...
    def clean(self, value, row=None, *args, **kwargs):
        ...
        row['geom'] = Point(row['lng'], row['lat'], srid=row.get('srid', 4326))
        # row['id'] = row['ID']
...
class CorrectionsWidget(ChoiceManyToMany):
...
class CorrectionField(Field):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(
            widget=CorrectionsWidget(type='corrections'),
            *args, **kwargs)
    ...
    def clean(self, value, row=None, *args, **kwargs):
        if value:
            instance, created = self.get_queryset(
                value, row, *args, **kwargs).get_or_create(**{self.field: value})
            return instance
        else:
            return None
