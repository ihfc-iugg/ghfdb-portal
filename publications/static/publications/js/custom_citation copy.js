...
function create_reference_card(bibtex) {
    ...
    var reference = Object.entries(data)

    for (let index = 0; index < reference.length; index++) {
        const element = reference[index];
        ...
        if (key == 'container-title') {
            ...
            value = `<a href='http://www.doi.org/${value}'>${value}</a>`
        } else if (key == 'URL') {
...
function add_row(key,value) {
    $(".referenceDetails").append(`<td>${capitalize(key)}:</td><td>${value}</td></tr>`)
