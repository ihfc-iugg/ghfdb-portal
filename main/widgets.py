...
class SiteWidget(ForeignKeyWidget):
    ...
    def clean(self,value,row=None):
        ...
        if not -90 <= float(row['lat']) <= 90:
            errors_dict['lat'] = ValidationError(_('latitude must be between -90 and 90 degrees'),code='invalid')
        if not -180 <= float(row['lng']) <= 180:
            errors_dict['lng'] = ValidationError(_('Longitude must be between -90 and 90 degrees'),code='invalid')
